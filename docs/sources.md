...
| `S67` | DeepSeek Harness CLI README | https://github.com/deepseek-ai/deepseek-harness/blob/master/apps/cli/README.md | `§Heading ¶n-m` or table name | Official DSH CLI entry modes, profile behavior, and workspace-root behavior. |
| `S68` | DeepSeek Harness CLI behavior reference | https://github.com/deepseek-ai/deepseek-harness/blob/master/apps/cli/reference/README.md | `§Heading ¶n-m` or table name | Official DSH profile boot, config dump, headless, permission, and workspace behavior reference. |
| `S69` | DeepSeek Harness agent-instructions package | https://github.com/deepseek-ai/deepseek-harness/blob/master/packages/context/agent-instructions/README.md | `§Heading ¶n-m` | Official DSH `AGENTS.md` loading, precedence, and instruction-budget behavior. |
| `S70` | `diskfont.library` autodocs index | https://developer.amigaos3.net/autodocs/diskfont.library/ | `item n` | Disk font (disk-font) open/close and `TextAttr`/`TextFont` function surface. |
| `S71` | `asl.library` autodocs | https://developer.amigaos3.net/autodocs/asl.library/ | `§<FunctionName>` (per-function page) | Classic ASL requester API: `AllocAslRequest`/`AslRequest`/`FreeAslRequest` semantics, the requester-structure read-only contract, and tag behavior. |
